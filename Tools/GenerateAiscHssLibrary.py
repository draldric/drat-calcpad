...
def build(workbook: Path) -> str:
    ...
    sections = sections[sections["Type"].eq("HSS")].reset_index(drop=True)
    if len(sections) != 714:
        raise ValueError(f"Expected 714 HSS sections; found {len(sections)}")
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        raise SystemExit("Usage: GenerateAiscHssLibrary.py SOURCE.xlsx OUTPUT.cpd|-")
